      ...
      >
        ...
        <div className="bg-[#002b36] rounded-t-3xl shadow-2xl max-h-[85vh] overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-4">
            ...
            {/* Content - Chevron View (Default) or Card View */}
            {!showHierarchyView ? (
              <div className="bg-[#073642] rounded-lg p-4 border border-[#586e75]">
                ...
                {/* Main Task Timeline */}
                {hierarchyTree && hierarchyTree.children && hierarchyTree.children.length > 0 ? (
                  <AsyncJobTimeline
                    jobName={hierarchyTree.title}
                    steps={hierarchyTree.children.map((child, index) => ({
                      id: child.task_id,
                      description: child.title,
                      shortLabel: child.title.length > 20 ? `${child.title.slice(0, 20)}...` : child.title,
                      detail: child.description,
                      estimatedMinutes: child.estimated_minutes || 0,
                      leafType: child.delegation_mode === 'auto' ? 'DIGITAL' : 'HUMAN',
                      icon: child.icon || child.custom_emoji || (child.delegation_mode === 'auto' ? '🤖' : '👤'),
                      status: 'pending' as const,
                      tags: [],
                      parentStepId: hierarchyTree.task_id,
                      level: child.level,
                    ...
                    onStepClick={(stepId) => {
                      ...
                      // Could handle expansion here if needed
                    }}
                  />
                ) : (
