...
// Import biological mode pages directly to fix hydration issues
...
export default function ADHDTaskManager() {
  ...
  const [biologicalMode, setBiologicalMode] = useState('capture'); // Start with Capture mode
  ...
  const [debugForceBanner, setDebugForceBanner] = useState(true);
  ...
  const [showHoldTutorial, setShowHoldTutorial] = useState(false);
  ...
  // Optimize initial data loading with useMemo and useCallback
  const fetchInitialData = useCallback(async () => {
    try {
      // Run all initial fetches in parallel instead of sequentially
      await Promise.all([
        fetchTasks(),
        fetchGameStats(),
        fetchEnergy()
  ...
  useEffect(() => {
    console.log('[MobileDebug] ADHDTaskManager mounted');
    return () => console.log('[MobileDebug] ADHDTaskManager unmounted');
  }, []);

  // Global error listeners for debugging blank screen issues
  useEffect(() => {
    const onError = (e: ErrorEvent) => {
      console.error('[MobileDebug] window.error:', e.message, e.error);
    };
    const onUnhandled = (e: PromiseRejectionEvent) => {
      console.error('[MobileDebug] unhandledrejection:', e.reason);
    };
  ...
  // Collect runtime render metrics
  useEffect(() => {
    const collect = () => {
      try {
        const el = rootRef.current;
        const rect = el?.getBoundingClientRect();
        const styles = el ? window.getComputedStyle(el) : ({} as any);
        setDebugInfo(prev => ({
          ...prev,
          time: new Date().toISOString(),
          viewport: { w: window.innerWidth, h: window.innerHeight },
          rootRect: rect ? { w: Math.round(rect.width), h: Math.round(rect.height) } : null,
        ...
        console.error('[MobileDebug] metrics error', e);
      }
    };
    collect();
  ...
  // ✅ CONNECTED: WebSocket for real-time updates with performance optimizations
  ...
  }, [API_URL, handleWebSocketMessage]);
  ...
  const fetchGameStats = useCallback(async () => {
    try {
      ...
      const [progressResult, gamificationResult] = await Promise.allSettled([
        fetch(`${API_URL}/api/v1/progress/level-progression?user_id=mobile-user`),
        fetch(`${API_URL}/api/v1/gamification/user-stats?user_id=mobile-user`)
      ]);

      // Handle progress API result
      ...
      // Handle gamification API result
      if (gamificationResult.status === 'fulfilled' && gamificationResult.value.ok) {
        const gamificationData = await gamificationResult.value.json();
  ...
  const fetchEnergy = useCallback(async () => {
    try {
      // ✅ CONNECTED: Real Energy API call with timeout
      ...
      // 🤖 Epic 2.2: Get AI energy predictions and recommendations (debounced and with error handling)
      debounce('energy-tracking', async () => {
        try {
          const trackController = new AbortController();
          const trackTimeoutId = setTimeout(() => trackController.abort(), 3000);
          ...
          // Silently handle AI energy tracking errors
  ...
  const toggleBodyDouble = () => setBodyDouble((v: boolean) => !v);
  ...
  // Performance monitoring removed - was causing 14+ second render times

  // Time of day detection for biological circuits (optimized)
  const updateTimeOfDay = useCallback(() => {
    ...
    setTimeOfDay(prev => prev !== newTimeOfDay ? newTimeOfDay : prev);
  }, []);
  ...
  // Performance monitoring effect - DISABLED to prevent performance issues
  // This useEffect was causing 14+ second render times, so it's disabled
  // useEffect(() => {
  ...
  //   
  //   // Only log if render time is significant and throttle logging
  //   if (renderTime > 100 && performanceRef.current.renderCount % 10 === 0) {
  ...
  const Pill = ({ text, tone }: { text: string; tone: 'good' | 'neutral' }) => (
    <span className={`px-2 py-1 rounded-full text-xs ${
      tone === 'good' 
        ? 'bg-[#859900] text-[#fdf6e3]' 
        : 'bg-[#586e75] text-[#93a1a1]'
    }`}>
      {text}
    </span>
  );
  ...
  return (
    <PerformanceOptimizer>
      <div ref={rootRef} className="fixed inset-0 bg-[#002b36] flex flex-col overflow-hidden">

        {/* Biological Mode Content - Routes to different pages based on active tab */}
        <div className="flex-1 overflow-hidden" id="mobile-content-root">
          {/* High-contrast test box to validate render path */}
          <div
            style={{
              position: 'absolute',
              top: 56,
      ...
      {/* High-priority debug banner to validate render path */}
      {debugForceBanner && (
        <div
          style={{
            position: 'fixed',
        ...
        >
          MOBILE PAGE RENDERED — DEBUG BANNER (click devtools console for [MobileDebug] logs)
        </div>
      )}
      ...
      {debugVisible && (
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
            <strong>MOBILE ROOT RENDERED</strong>
            <button
              onClick={() => setDebugVisible(false)}
              style={{ color: '#0ff', border: '1px solid #0ff', borderRadius: 4, padding: '0 6px', background: 'transparent' }}
            >hide</button>
          </div>
