...
// Lazy load biological mode pages for better performance
const CapturePage = lazy(() => import('./capture/page'));
const ScoutPage = lazy(() => import('./scout/page'));
const HunterPage = lazy(() => import('./hunter/page'));
const MenderPage = lazy(() => import('./mender/page'));
const MapperPage = lazy(() => import('./mapper/page'));
...
export default function ADHDTaskManager() {
  ...
  const MemoizedCapturePage = useMemo(() => (
    <CapturePage
      onTaskCaptured={() => {
    ...
    />
  ), [fetchTasks, fetchGameStats]);

  const MemoizedScoutPage = useMemo(() => (
    <ScoutPage
      onTaskTap={handleTaskTap}
      refreshTrigger={refreshTrigger}
    />
  ), [handleTaskTap, refreshTrigger]);

  const MemoizedHunterPage = useMemo(() => (
    <HunterPage
      ...
      refreshTrigger={refreshTrigger}
    />
  ), [handleSwipeLeft, handleSwipeRight, handleTaskTap, refreshTrigger]);

  const MemoizedMenderPage = useMemo(() => (
    <MenderPage
      energy={energy}
      onTaskTap={handleTaskTap}
      refreshTrigger={refreshTrigger}
    />
  ), [energy, handleTaskTap, refreshTrigger]);

  const MemoizedMapperPage = useMemo(() => (
    <MapperPage
      xp={xp}
      level={level}
    ...
    <PerformanceOptimizer>
      <div className="fixed inset-0 bg-[#002b36] flex flex-col overflow-hidden">
        ...
        <div className="flex-1 overflow-hidden">
          <ErrorBoundary>
            <Suspense fallback={
              <div className="flex items-center justify-center h-full bg-[#002b36]">
                <div className="text-center">
                  <div className="animate-spin w-8 h-8 border-2 border-[#2aa198] border-t-transparent rounded-full mx-auto mb-2"></div>
                  <div className="text-[#93a1a1] text-sm">Loading...</div>
                </div>
              </div>
            }>
              {biologicalMode === 'capture' && MemoizedCapturePage}
              {biologicalMode === 'scout' && MemoizedScoutPage}
              {biologicalMode === 'hunter' && MemoizedHunterPage}
              {biologicalMode === 'mender' && MemoizedMenderPage}
              {biologicalMode === 'mapper' && MemoizedMapperPage}
            </Suspense>
          </ErrorBoundary>
        </div>
