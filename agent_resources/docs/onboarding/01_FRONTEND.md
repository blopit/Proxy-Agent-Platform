...
- **Icons**: lucide-react-native (UI icons) + OpenMoji (emoji display)
- **Emoji Standard**: OpenMoji black/white for consistent, accessible emoji rendering
...
├── challenges.tsx       → /onboarding/challenges (Step 3)
├── adhd-support.tsx     → /onboarding/adhd-support (Step 4)
...
- Getting started on tasks (`task_initiation`)
- Staying focused (`focus`)
- Time awareness (`time_blindness`)
- Keeping things organized (`organization`)
- Managing overwhelm (`overwhelm`)
- Switching between tasks (`transitions`)
- Deciding what to do first (`prioritization`)
- Getting too focused on one thing (`hyperfocus`)
...
**File**: `mobile/src/components/ui/OpenMoji.tsx`

**Purpose**: Consistent emoji rendering across all onboarding screens

**Props**:
```typescript
interface OpenMojiProps {
  emoji?: string;        // Emoji character (e.g., "🎯")
  code?: string;         // Unicode codepoint (e.g., "1F3AF")
  size?: number;         // Size in pixels (default: 24)
  color?: string;        // Color override (native emojis don't support this)
  style?: any;           // Optional style override
}
```

**Visual Design**:
- Uses native platform emoji rendering for maximum compatibility
- Wraps emoji in View container for consistent sizing
...
**Important**: All emoji displays in onboarding MUST use OpenMoji component instead of plain Text elements for consistency and accessibility.
