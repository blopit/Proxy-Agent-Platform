...
from ..tools import auto_search, semantic_search
...
class TestEndToEndSearch:
    ...
    @pytest.mark.asyncio
    async def test_complete_semantic_search_workflow(
    ...
    async def test_complete_hybrid_search_workflow(self, test_dependencies, sample_hybrid_results):
        ...
        async def hybrid_workflow(messages, tools):
            ...
            if call_count == 1:
                return ModelTextResponse(
                    content="I'll perform a hybrid search combining semantic and keyword matching."
                )
            elif call_count == 2:
        ...
        assert (
            "Find exact Python syntax examples" in deps.query_history or len(deps.query_history) > 0
        )
        ...
        with patch.dict(
            ...
            with patch("asyncpg.create_pool") as mock_create_pool:
                with patch("openai.AsyncOpenAI") as mock_openai:
                    ...
                    assert (
                        deps.settings.database_url == "postgresql://test:test@localhost:5432/testdb"
                    )
    ...
    async def test_tools_to_agent_integration(self, test_dependencies, sample_search_results):
        ...
        async def multi_tool_workflow(messages, tools):
            ...
            if call_count == 1:
                ...
                return {
                    "hybrid_search": {"query": "test hybrid", "match_count": 5, "text_weight": 0.4}
                }
            elif call_count == 3:
    ...
    async def test_research_workflow_scenario(self, test_dependencies):
        ...
        async def research_workflow(messages, tools):
            ...
            if call_count % 2 == 1:  # Analysis calls
                return ModelTextResponse(
                    content="I'll search for research information on this topic."
                )
            else:  # Tool calls
                query_idx = (call_count // 2) - 1
                if query_idx < len(research_queries):
                    return {
                        "auto_search": {"query": research_queries[query_idx], "match_count": 10}
                    }
                else:
