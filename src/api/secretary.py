...
async def get_secretary_dashboard(
    user_id: str | None = Query(None, description="User ID for filtering tasks"),
...
async def get_daily_briefing(
    ...
    try:
        if time_of_day not in ["morning", "evening"]:
            raise HTTPException(
                status_code=400, detail="time_of_day must be 'morning' or 'evening'"
            )
...
async def mark_task_urgent(
    task_id: str, secretary_service: SecretaryService = Depends(get_secretary_service)
) -> dict[str, Any]:
    """Quick action to mark a task as urgent"""
    try:
        # This would integrate with the task service to update priority
        # For now, return a success response
        return {"success": True, "message": f"Task {task_id} marked as urgent", "task_id": task_id}
